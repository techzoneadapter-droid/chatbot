...
export function ProviderSettings() {
  ...
  async function test(provider: Provider) {
    ...
    const response = await fetch("/api/ai/providers", {
      ...
      body: JSON.stringify({
        provider,
        model: provider === "gemini" ? "gemini-3.8-flash" : metaModel.trim()
      })
    });
    const data = await response.json().catch(() => ({}));
    setWorking(null);
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="panel rounded-lg p-4">
        ...
        <p className="mt-2 text-xs text-muted">Meta có thể thay đổi danh sách model, nên hệ thống không khóa cứng tên model. Hãy dùng đúng Model ID mà tài khoản Meta của bạn được cấp.</p>
        <div className="mt-4 flex flex-wrap gap-2">
